...
const card = { background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 12 }
const sectionH = { fontFamily: "'Playfair Display', Georgia, serif", fontSize: 15, fontWeight: 600, color: 'var(--text)', marginBottom: 16 } as const
        ...
        <div className="rg-2" style={{ gap: 20, marginBottom: 24 }}>
          ...
          <div style={{ ...card, padding: 20 }}>
            ...
            {todayTasks.map((t, i) => {
              ...
              return (
                <div key={t.id} style={{ ...rowItem, paddingBottom: isLast ? 0 : 11, marginBottom: isLast ? 0 : 11, borderBottom: isLast ? 'none' : '1px solid var(--border-3)' }}>
                ...
                return (
                  <div key={ig.id} style={{ display: 'flex', alignItems: 'center', gap: 8, paddingBottom: isLast ? 0 : 10, marginBottom: isLast ? 0 : 10, borderBottom: isLast ? 'none' : '1px solid var(--border-3)' }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <p style={{ fontSize: 12, fontWeight: 600, color: 'var(--text)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{ig.handle}</p>
                      <p style={{ fontSize: 11, color: 'var(--text-3)' }}>{ig.engagementRate}% eng.</p>
                    </div>
                    <span style={{
          ...
          <div style={{ ...card, padding: 20 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {payments.filter(p => p.status !== 'pago').slice(0, 3).map((p, i) => {
                const isLast = i === 2
                return (
                  <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 8, paddingBottom: isLast ? 0 : 8, borderBottom: isLast ? 'none' : '1px solid var(--border-3)' }}>
                    <span style={{ flex: 1, fontSize: 12, color: 'var(--text)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{p.clientName}</span>
                    <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--text)' }}>{formatCurrency(p.value)}</span>
            ...
            {thisWeekPosts.map((item, i) => {
              ...
              return (
                <div key={item.id} style={{ ...rowItem, paddingBottom: isLast ? 0 : 11, marginBottom: isLast ? 0 : 11, borderBottom: isLast ? 'none' : '1px solid var(--border-3)' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={{ fontSize: 13, fontWeight: 500, color: 'var(--text)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{item.caption}</p>
                    <p style={{ fontSize: 11, color: 'var(--text-3)', marginTop: 2 }}>{item.clientName} · {formatDate(item.date)}</p>
                  </div>
              ...
              <thead>
                <tr style={{ borderBottom: '1px solid var(--border-3)' }}>
                  {['Cliente', 'Ref.', 'Valor', 'Status'].map(h => (
                    <th key={h} style={{ fontSize: 10, fontWeight: 700, color: 'var(--text-3)', textAlign: 'left', paddingBottom: 8, letterSpacing: '0.05em', textTransform: 'uppercase' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {recentPayments.map((p, i) => {
                  ...
                  return (
                    <tr key={p.id} style={{ borderBottom: isLast ? 'none' : '1px solid var(--border-3)' }}>
                      <td style={{ fontSize: 13, color: 'var(--text)', padding: '9px 0', fontWeight: 500 }}>{p.clientName}</td>
                      <td style={{ fontSize: 12, color: 'var(--text-2)', padding: '9px 0' }}>{p.reference}</td>
                      <td style={{ fontSize: 13, color: 'var(--text)', padding: '9px 0', fontWeight: 700 }}>{formatCurrency(p.value)}</td>
